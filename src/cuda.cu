...
__global__ void kernel_stage1(int width, int channels) {
    ...
    if (threadIdx.x < channels && threadIdx.y == 0) {
        d_mosaic_sum[tile_index + threadIdx.x] = block_pixel[threadIdx.x];
    }
...
__global__ void kernel_stage2(int channels) {
    const unsigned int tile_index = blockIdx.x * blockDim.x + threadIdx.x;
    // const unsigned int tile_offset = blockIdx.x * TILE_SIZE * TILE_SIZE * channels;
    
    __shared__ unsigned long long block_sum[4];

    d_mosaic_value[tile_index + 0] = (unsigned char) (d_mosaic_sum[tile_index + 0] / TILE_PIXELS);
    d_mosaic_value[tile_index + 1] = (unsigned char) (d_mosaic_sum[tile_index + 1] / TILE_PIXELS);
    d_mosaic_value[tile_index + 2] = (unsigned char) (d_mosaic_sum[tile_index + 2] / TILE_PIXELS);
    // d_mosaic_value[tile_index + 3] = (unsigned char) (d_mosaic_sum[tile_index + 3] / TILE_PIXELS);

    atomicAdd(&(block_sum[0]), d_mosaic_value[tile_index + 0]);
    atomicAdd(&(block_sum[1]), d_mosaic_value[tile_index + 1]);
    atomicAdd(&(block_sum[2]), d_mosaic_value[tile_index + 2]);
    // atomicAdd(&(block_sum[3]), d_mosaic_value[tile_index + 3]);
...
__global__ void kernel_stage3(int width, int channels) {
    const unsigned int tile_index = blockIdx.x * channels;
    const unsigned int tile_offset = blockIdx.x * TILE_SIZE * TILE_SIZE * channels;
    ...
    if (threadIdx.x < channels){
        block_pixel[threadIdx.x] = d_mosaic_value[tile_index + threadIdx.x];
    }
    ...
    d_output_image_data[tile_index + pixel_offset +0] = block_pixel[0];
    d_output_image_data[tile_index + pixel_offset +1] = block_pixel[1];
    d_output_image_data[tile_index + pixel_offset +2] = block_pixel[2];
...
void cuda_stage1() {
    ...
    kernel_stage1<<< grid_size, block_size>>>(cuda_input_image.width, cuda_input_image.channels);
#ifdef VALIDATION
    // TODO: Uncomment and call the validation function with the correct inputs
    // You will need to copy the data back to host before passing to these functions
    // (Ensure that data copy is carried out within the ifdef VALIDATION so that it doesn't affect your benchmark results!)
    CUDA_CALL(cudaMemcpy(cpu_mosaic_sum, d_mosaic_sum, cuda_TILES_X * cuda_TILES_Y * input_image->channels * sizeof(unsigned long long), cudaMemcpyDeviceToHost));
...
void cuda_stage2(unsigned char* output_global_average) {
    ...
    kernel_stage2<<< grid_size, block_size>>>(cuda_input_image.channels);
#ifdef VALIDATION
    // TODO: Uncomment and call the validation functions with the correct inputs
    // You will need to copy the data back to host before passing to these functions
    // (Ensure that data copy is carried out within the ifdef VALIDATION so that it doesn't affect your benchmark results!)
    CUDA_CALL(cudaMemcpy(cpu_mosaic_value, d_mosaic_value, cuda_TILES_X * cuda_TILES_Y * input_image->channels * sizeof(unsigned char), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(output_global_average, d_output_global_average, 4, cudaMemcpyDeviceToHost));
...
void cuda_stage3() {
    ...
    kernel_stage3<<< grid_size, block_size>>>(cuda_input_image.width, cuda_input_image.channels);
